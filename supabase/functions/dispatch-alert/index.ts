...
Deno.serve(async (req) => {
  ...
  for (const alert of alerts as Alert[]) {
    ...
    channelUpdates.whatsapp_status = wa.status === 'fulfilled' ? 'sent' : 'failed'
    channelUpdates.email_status    = em.status === 'fulfilled' ? 'sent' : 'failed'
    channelUpdates.slack_status    = sl.status === 'fulfilled' ? 'sent' : 'failed'
    channelUpdates.telegram_status = tg.status === 'fulfilled' ? 'sent' : 'failed'

    if (wa.status === 'rejected') channelUpdates.whatsapp_error  = (wa.reason as Error).message
    if (em.status === 'rejected') channelUpdates.email_error     = (em.reason as Error).message
    if (sl.status === 'rejected') channelUpdates.slack_error     = (sl.reason as Error).message
    if (tg.status === 'rejected') channelUpdates.telegram_error  = (tg.reason as Error).message
...
function formatTelegramMessage(alert: Alert, order?: BostaOrder | null): string {
  const title = alert.event_type.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
  ...
  const lines = [
    `${severityPart}  ·  <b>${title}</b>`,
    '',
    body,
  ]

  if (order) {
    const name    = order.receiver?.fullName || [order.receiver?.firstName, order.receiver?.lastName].filter(Boolean).join(' ')
    const phone   = order.receiver?.phone
    const addr    = [order.dropOffAddress?.firstLine, order.dropOffAddress?.city?.name].filter(Boolean).join(' — ')
    const ref     = order.businessReference
    const cod     = order.cod
    const attempts = order.deliveryAttemptsLength
    const pkg     = [order.specs?.packageDetails?.itemsCount != null ? `${order.specs.packageDetails.itemsCount} item(s)` : null, order.specs?.packageType].filter(Boolean).join(' · ')
...
async function sendTelegram(alert: Alert): Promise<void> {
  ...
  if (trackingNumber) {
    payload.reply_markup = {
      inline_keyboard: [[
        { text: '📦 Track Order', url: `https://business.bosta.co/orders/${trackingNumber}` },
      ]],
    }
  }
  ...
  const res = await fetch(SLACK_WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      blocks: [
        ...
        {
          type: 'context',
          elements: [{ type: 'mrkdwn', text: `Source: *${alert.source}* | ${alert.created_at}` }],
        },
      ],
    }),
  })

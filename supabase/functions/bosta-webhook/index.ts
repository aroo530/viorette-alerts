...
function classifyException(code?: number): { severity: Severity; label: string } {
  ...
  const map: Record<number, { severity: Severity; label: string }> = {
    ...
    30:  { severity: 'critical', label: 'Order opened when it should not have been' },
  }
...
function buildAlert(payload: BostaPayload): AlertResult {
  ...
  const STATE_MAP: Record<number, () => AlertResult> = {
    10:  () => ({ eventType: 'pickup_requested',      severity: 'info',     message: `Pickup requested for ${tn}.` }),
    11:  () => ({ eventType: 'waiting_for_route',     severity: 'info',     message: `${tn} waiting for route (Cash Collection).` }),
    20:  () => ({ eventType: 'route_assigned',        severity: 'info',     message: `Route assigned for ${tn}.` }),
    21:  () => ({ eventType: 'picked_up_from_biz',    severity: 'info',     message: `${tn} picked up from business.` }),
    22:  () => ({ eventType: 'pickup_from_consignee', severity: 'info',     message: `${tn} heading to consignee for pickup (${type}).` }),
    23:  () => ({ eventType: 'picked_up_from_cons',   severity: 'info',     message: `${tn} picked up from consignee (${type}).` }),
    24:  () => ({ eventType: 'received_at_warehouse', severity: 'info',     message: `${tn} received at warehouse.` }),
    25:  () => ({ eventType: 'shipment_fulfilled',    severity: 'info',     message: `${tn} fulfilled.` }),
    30:  () => ({ eventType: 'in_transit',            severity: 'info',     message: `${tn} in transit between hubs.` }),
    40:  () => ({ eventType: 'picking_up_cash',       severity: 'info',     message: `${tn} picking up cash from end customer.` }),
    41:  () => ({ eventType: 'out_for_delivery',      severity: 'info',     message: `${tn} out for delivery/return. Attempt ${attempts + 1}.` }),
    ...
    46:  () => ({ eventType: 'shipment_returned',     severity: 'medium',   message: `${tn} returned to sender (${type}).` }),
    49:  () => ({ eventType: 'shipment_canceled',     severity: 'medium',   message: `${tn} canceled.` }),
    60:  () => ({ eventType: 'returned_to_stock',     severity: 'low',      message: `${tn} returned to stock.` }),
    ...
    48:  () => ({ eventType: 'shipment_terminated',   severity: 'critical', message: `${tn} TERMINATED after ${attempts} failed attempts.` }),
    100: () => ({ eventType: 'shipment_lost',         severity: 'critical', message: `${tn} marked as LOST.` }),
    101: () => ({ eventType: 'shipment_damaged',      severity: 'critical', message: `${tn} marked as DAMAGED.` }),
    102: () => ({ eventType: 'under_investigation',   severity: 'high',     message: `${tn} under investigation.` }),
    103: () => ({ eventType: 'action_required',       severity: 'high',     message: `${tn} awaiting your action — return failed after 3 attempts.` }),
    104: () => ({ eventType: 'shipment_archived',     severity: 'low',      message: `${tn} archived.` }),
    105: () => ({ eventType: 'shipment_on_hold',      severity: 'medium',   message: `${tn} placed on hold.` }),
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ received: true, alert_id: inserted.id, event_type: eventType, severity }),
